...
func TestSelectBuilder(t *testing.T) {
	...
	testCases := []struct {
		...
		// If
		{users.Select("*", users.C("foo").Gt(users.C("bar")).If(users.C("qux"), users.C("bod")).As("expr")).Where(foo.InTuple(&Subquery{objects.Select(objects.C("foo")).Where(objects.C("foo").Gt(10))})),
